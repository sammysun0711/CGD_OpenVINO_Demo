...
def process_sop_data(data_path):
    ...
    for data_type, image_list in data_tuple.items():
        for index, line in tqdm(enumerate(open('{}/Ebay_{}.txt'.format(data_path, data_type), 'r', encoding='utf-8')), desc='processing {} data for sop dataset'.format(data_type)):
            if index != 0:
    ...
    print("Save processed sop data in {}/uncropped_data_dicts.pth".format(data_path))
